...
transform.grayscale = function (bmp, callback) {
  let altBuffer = Buffer.from(bmp.allData), grayInt, j = 54;
  do {
    grayInt = (altBuffer.readUInt8(j) + altBuffer.readUInt8(j + 1) + altBuffer.readUInt8(j + 2)) / 3;
    altBuffer.writeUInt8(grayInt, j), altBuffer.writeUInt8(grayInt, j + 1), altBuffer.writeUInt8(grayInt, j + 2);
